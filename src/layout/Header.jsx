...
export const SubHeader = () => {
  ...
  const [state, dispatch] = useReducer(reducerModal, { content: "NOT" });
  const handleActiveIcon = (id) => setActiveIcon(id);
  const handleModal = (type) => {
    setIsOpen((prev) => !prev);
    setActiveIcon(IsOpen ? ActiveIcon : 0);
    dispatch({ type });
  };
    ...
    <Fragment>
      {IsOpen && (
        <Modal setIsOpen={setIsOpen}>
          <ModalContent title={"AFFILIATES"} setIsOpen={setIsOpen}>
            {state?.content}
          </ModalContent>
        </Modal>
      )}
      <header className="bg-[#131620] py-6 px-8 flex items-center justify-between">
        <Image
          className={classNames(DashboardStyle["Home__header-img"])}
          src={LogoText}
          srcSet={LogoText}
        />
        ...
        <div className="flex-grow text-center space-x-6">
          <button
            className={`${
              ActiveIcon === 1 ? "text-[#EC981A]" : "text-[#4d5b97]"
            } hover:text-[#EC981A] text-xl border-none outline-none bg-transparent`}
            onClick={() => {
              handleActiveIcon(1);
              handleModal("AFFILIATES");
            }}
          >
          ...
          <button
            className={`${
              ActiveIcon === 2 ? "text-[#EC981A]" : "text-[#4d5b97]"
            } hover:text-[#EC981A] text-xl border-none outline-none bg-transparent`}
            onClick={() => {
              handleActiveIcon(2);
              handleModal("FREE_COINS");
            }}
          >
          ...
          <button
            className={`${
              ActiveIcon === 3 ? "text-[#EC981A]" : "text-[#4d5b97]"
            } hover:text-[#EC981A] text-xl border-none outline-none bg-transparent`}
            onClick={() => {
              handleActiveIcon(3);
              handleModal("PROFILE");
          ...
          <Link
            to={"/daily-cases"}
            className={`${
              ActiveIcon === 4 ? "text-[#EC981A]" : "text-[#4d5b97]"
            } hover:text-[#EC981A] text-xl`}
            onClick={() => handleActiveIcon(4)}
          >
        ...
        <div className="flex items-center gap-2.5">
          <button
            type="button"
            className="inline-flex p-3 bg-[#20273d] items-center gap-2"
          >
